...
function drawThreeGeo(json, radius, shape, options) {
    ...
    var json_props = createPropertyArray(json); 
    ...
    for (var geom_num = 0; geom_num < json_geom.length; geom_num++) {
        ...
        if (json_geom[geom_num].type == 'Point') {
            ...
            drawParticle(y_values[0], z_values[0], x_values[0], options, json_props[geom_num]);
            ...
            var dist = drone.localToWorld(new THREE.Vector3(0, 0, 0)).distanceTo(new THREE.Vector3(y_values[0], z_values[0], x_values[0] ));
            if(dist < 10) drawLine(y_values, z_values, x_values, options); else clearArrays();
...
function createPropertyArray(json) {
    ...
    if (json.type == 'Feature') {
        property_array.push(json.properties);        
    } else if (json.type == 'FeatureCollection') {
        for (var feature_num = 0; feature_num < json.features.length; feature_num++) { 
            property_array.push(json.features[feature_num].properties);
        }
    } else {
        throw new Error('The geoJSON is not valid.');
    }    
...
function drawParticle(x, y, z, particle_material, props) {
    ...
    /*var mesh = new THREE.Mesh(beer_geom, beer_mat);
